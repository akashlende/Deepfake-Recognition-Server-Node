...
class VideoReader:
    ...
    def read_frames(self, path, num_frames=-1, jitter=0, seed=None):
    ...
    try:
        ...
        if len(processed_frames) > 0:
            ...
            if n > 0:
                ...
                video_result_path1 = os.path.join(os.getcwd(), "video-cache", "immediate-video", file_name + '.mp4')
                video_result_path2 = os.path.join(os.getcwd(), "video-results", "video", file_name + '.mp4')
                ...
                for i, frame_data in enumerate(processed_frames):
                    ...
                    if writer is None:
                        writer = cv2.VideoWriter(video_result_path1, fourcc, fps,
                                                 (width, height))
                    for bbox in frame_data["boxes"]:
                        ...
                        output_list = ['{0:.2f}'.format(score), '{0:.2f}'.format(1.0 - score)]
                        cv2.putText(image, str(output_list)+'=>'+label, (xmin, ymin+h+30),
                ...
                subprocess.call(['ffmpeg', '-y', '-i', f'{video_result_path1}', '-i',
                                f'{file_name}.mp3', '-c:v', 'copy', '-c:a', 'aac', video_result_path2])
...
def predict_image(model, path, processor, face_extractor):
    detector = MTCNN(margin=0, thresholds=[
                              0.7, 0.8, 0.8], device="cuda")
    file_name = path.split('/')[-1]
    new_image = cv2.imread(path)
    frame = processor._postprocess_frame(new_image)
    h, w = frame.shape[:2]
    img = Image.fromarray(frame.astype(np.uint8))
    img = img.resize(size=[s // 2 for s in img.size])
    ...
    if l == 1:
        l = 2
    x = np.zeros((l, 380, 380, 3), dtype=np.uint8)

    n = 0
    for i, face in enumerate(faces):
        resized_face = isotropically_resize_image(face, 380)
        resized_face = put_to_center(resized_face, 380)
        x[i] = resized_face
        n = i + 1
    ...
    cv2.imwrite(os.path.join(os.getcwd(), "image-cache", "result", file_name), image)
    return np.mean(strategy(faces_pred))